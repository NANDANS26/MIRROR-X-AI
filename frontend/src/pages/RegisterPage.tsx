/**
 * RegisterPage.tsx — Neural synthesis / system initialization scene.
 *
 * Background: Deep violet crystalline lattice with animated data nodes,
 * emergence pulses, and binary stream columns. Pure CSS/canvas — no Three.js.
 *
...
function RegisterBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Binary streams (Matrix-style, violet-cyan)
      ...
      // Emergence rings from bottom-center (creation motif)
      for (let r = 0; r < 4; r++) {
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(msg?.toLowerCase().includes('already') ? 'This email is already registered.' : msg ?? 'Registration failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#020010' }}>
      ...
      >
        <div style={{
          ...
          {/* Init progress bar decoration */}
          <div style={{ marginBottom: 26, display: 'flex', flexDirection: 'column', gap: 5 }}>
            {['System check', 'Node allocation', 'Access provisioning'].map((label, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 9, color: 'rgba(140,100,220,0.55)', letterSpacing: '0.12em', textTransform: 'uppercase', minWidth: 80 }}>{label}</span>
              </div>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              { id: 'reg-email', label: 'Identity (Email)', type: 'email', value: email, onChange: setEmail, auto: 'email' },
              { id: 'reg-password', label: 'Access Key (8–128 chars)', type: 'password', value: password, onChange: setPassword, auto: 'new-password' },
              { id: 'reg-confirm', label: 'Confirm Access Key', type: 'password', value: confirm, onChange: setConfirm, auto: 'new-password' },
            ].map((f, i) => (
              <motion.div
                key={f.id}
